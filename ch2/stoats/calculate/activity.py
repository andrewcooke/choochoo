...
from ..names import ELEVATION, DISTANCE, M, POWER_ESTIMATE, HEART_RATE, ACTIVE_DISTANCE, MSR, SUM, CNT, MAX, \
    ...
    SPHERICAL_MERCATOR_Y, DIRECTION, DEG, ASPECT_RATIO, FITNESS_D_ANY, FATIGUE_D_ANY, _d
...
class ActivityCalculator(ActivityJournalCalculatorMixin, DataFrameCalculatorMixin, MultiProcCalculator):
    ...
    def _copy_results(self, s, ajournal, loader, data):
        ...
        self.__copy_all(ajournal, loader, stats, _d(FATIGUE_D_ANY), None, summaries(MAX, MSR), ajournal.start)
        self.__copy_all(ajournal, loader, stats, _d(FITNESS_D_ANY), None, summaries(MAX, MSR), ajournal.start)
        if climbs:
