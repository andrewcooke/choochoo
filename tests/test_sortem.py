...
@@@@@%######%%%%@ .-=*%@.-=*#%%%%@@@.:-+#@ ..-+**#%@ :-=+*++''', scale=10, step=1/10)
        ...
        with self.bilinear() as bilinear:
            ...
            with self.spline() as spline:

                class Delta:

                    def elevation(self, x, y):
                        b = bilinear.elevation(x, y)
                        s = spline.elevation(x, y)
                        delta = b - s
                        return delta

                self.assert_contours(Delta(), -33.5, -70.5, 30,
'''
 .@ .% .%                     -@ .  .@    .@  @     .  @. .@
:-@.:@ : :::..... :..-....   ..@  %@.+%@#% #@ *%@ @  @@* @  
 :*@ %@ %@  @@@@@%           .= .:.:- .. .:  .  ..  . .@-. :
...
. =.... :..:  -% +*  %   :- .::..: .- := :=% : .:= :+@.-% @+
     .        -% =# .@ .%  @  @     -% :% .@ .#  .  -% .@  .
@@@%%  @@@  @ .@ =%.-% :+% %%@# @@@@-=% +%@%%..#@ @  % .%@%@
.... -:....:... .-# :*@.*%.*@  ..:.:- .=%.: .-# .- :+ :=   :''', scale=1e-10, step=1/10)

            with self.spline(1e8) as spline:
...
.    @       @   .:-.        @ @@@%#       @%      @      #%
   ...
   ..          @@@ @ ..     .. @@   : @@ ...@ .     @@%  :  ''', scale=3, step=1/10)
            ...
            with self.spline(1e10) as spline:
...
*+==-.   @@%%%#**#%%@  .:-----:  @%#**+****+==++=------===-:''', scale=10, step=1/10)
    ...
    def test_interp_4_tiles(self):
        ...
        with self.bilinear() as oracle:
            self.assert_contours(oracle, -34-ten_arcsec, -71-2*ten_arcsec, 20,
'''
#**#%@ :.  :-+=+=: @@%@@@@ .-=+*+++=--:.
#**#%@.::.::-=*+-:. @@@ .  .:-+****++++-
%#*##@.:::--=+*+=:.  @  :...:=+##%##****
