...
class TestFixFit(TestCase, OutputMixin):
    ...
    def test_null(self):
        good = read_fit(self.log, 'data/test/source/personal/2018-08-27-rec.fit')
    ...
    def test_drop(self):
        bad = read_fit(self.log, 'data/test/source/other/8CS90646.FIT')
    ...
    def test_unknown_bad(self):
        bad = read_fit(self.log, 'data/test/source/other/2018-04-15-09-18-20.fit')
        fixed = fix(self.log, bytearray(bad), drop=True)
        with self.assertTextMatch('data/test/target/other/TestFixFit.test_unknown_bad:1') as output:
            summarize(self.log, RECORDS, fixed, output=output)
        bad = read_fit(self.log, 'data/test/source/other/2018-02-24-10-04-10.fit')
        fixed = fix(self.log, bytearray(bad), drop=True)
        with self.assertTextMatch('data/test/target/other/TestFixFit.test_unknown_bad:2') as output:
            summarize(self.log, RECORDS, fixed, output=output)

    def test_unknown_good(self):
        good = read_fit(self.log, 'data/test/source/other/77F73023.FIT')
        same = fix(self.log, bytearray(good))
        self.assertEqual(good, same)
        self.assertFalse(good is same)

    def test_scaled(self):
        bad = read_fit(self.log, 'data/test/source/other/2018-05-30-22-00-44.fit')
        fixed = fix(self.log, bytearray(bad), drop=True, max_drop_cnt=2)
        with self.assertTextMatch('data/test/target/other/TestFixFit.test_scaled') as output:
