...
def decode_all(log, path, messages):
    with open(path, 'rb') as input:
        data = input.read()
    offset, header = read_header(log, data, messages)
    check_crc(log, data[offset:])
    yield header
...
def read_header(log, data, messages):
    ...
    size = len(data) - offset - 2
    if size != message['data_size']:
        raise Exception('Bad data lenth (%d / %d)' % (size, message['data_size']))
    log.debug('Checked data length')
...
def check_crc(log, data):
    ...
    log.debug('Checked data checksum')
